...
def set_playback_mode(enabled: bool) -> None:
    """Faster cursor motion during recorded timeline replay."""
    global _playback_mode
    _playback_mode = enabled


def _motion_scale() -> float:
    # Playback: snappy (~250–450ms moves). Live explore keeps human pace.
    return 0.4 if _playback_mode else MOTION_SCALE


def _frame_steps_for_duration(duration_ms: float) -> int:
    """Hop count for a move/pause so screenshare stays near TARGET_FPS."""
    d = max(0.0, _scaled_ms(duration_ms))
    if d <= 0:
        return 1
    # Screenshare JPEG is expensive — never dump 60 screenshots into one move.
    # The ceiling also protects the live audio path: every hop is a synchronous
    # encode in the same process that has to keep 24 kHz PCM flowing.
    if _playback_mode:
        return max(6, min(12, int(d / 40.0) or 6))
    return max(10, min(14, int(d / FRAME_MS)))
...
#: only pushed frames are. Steps scale to TARGET_FPS for smooth motion.
FRAME_STEPS = 20
...
def _paced_wait(
    ...
    if total <= 0:
        on_frame()
        return
    hops = _frame_steps_for_duration(total)
    slice_ms = total / hops
    for _ in range(hops):
        on_frame()
        if slice_ms >= 1:
    ...
    *,
    ...
    page.evaluate(
        """async ([x, y, shortMs, longMs, shortD, longD, ease, scale]) => {
          ...
          // Quadratic mid-point with light jitter — not a straight teleport.
          const ctrlX = x0 + (x - x0) * 0.5 + (Math.random() - 0.5) * dist * 0.2;
          const ctrlY = y0 + (y - y0) * 0.5 + (Math.random() - 0.5) * dist * 0.2;
          const anim = c.animate(
            [
              { left: x0 + 'px', top: y0 + 'px' },
              { left: ctrlX + 'px', top: ctrlY + 'px' },
              { left: x + 'px', top: y + 'px' },
            ],
...
def _move_stepped(
    ...
    ctrl_x = x0 + (x - x0) * 0.5
    ctrl_y = y0 + (y - y0) * 0.5
    steps = _frame_steps_for_duration(duration)
    hop_ms = duration / steps
...
def _highlight_box(page: Page, box: dict[str, float]) -> None:
    ...
    Playwright selectors (`:has-text`, `>>`, etc.) are not CSS; passing them to
    document.querySelector throws and aborts login/clicks mid-demo.
    """
    page.evaluate(
        """(b) => {
          const h = document.getElementById('nav-cursor-highlight');
          if (!h || !b) return;
          h.style.left = (b.x - 4) + 'px';
          h.style.top = (b.y - 4) + 'px';
          h.style.width = (b.width + 8) + 'px';
          h.style.height = (b.height + 8) + 'px';
          h.style.opacity = '1';
        }""",
...
def _snap_cursor(page: Page, x: float, y: float) -> None:
    """Place overlay at (x, y) with no animation — used for recorded path hops."""
    page.evaluate(
        """([x, y]) => {
          const c = document.getElementById('nav-cursor');
          if (!c) return;
...
def replay_mouse_path(
    page: Page,
    points: list[dict[str, int]],
    *,
    on_frame: Callable[[], None] | None = None,
) -> tuple[float, float]:
    """Replay recorded path as one smooth move to the click point.

    Point-by-point replay + per-hop screenshots froze Chromium on Meet
    screenshare. Keep the recorded *endpoint* (where the Client clicked) and
    animate there in a handful of frames.
    """
    install_cursor(page)
    ...
    x0 = float(start.get("x") or 0)
    y0 = float(start.get("y") or 0)
    ...
    if len(points) >= 3 and on_frame is not None:
        mid = points[len(points) // 2]
        mx = float(mid.get("x") or x1)
        my = float(mid.get("y") or y1)
        _move_stepped(page, mx, my, on_frame)
    if on_frame is not None:
        _move_stepped(page, x1, y1, on_frame)
    else:
...
def _play_click_fx(page: Page, x: float, y: float) -> None:
    page.evaluate(
        ...
        [x, y, RIPPLE_MS, PRESS_MS, _motion_scale()],
    )
